// GERADO a partir de lib/calculadora-termica.ts (motor do site, ASTM C680 / ISO 12241 / ABNT NBR 16281). Não editar aqui.
"use strict";
...
exports.COMBUSTIVEIS = void 0;
exports.calcularK = calcularK;
exports.calcularHConv = calcularHConv;
exports.encontrarTemperaturaFaceFria = encontrarTemperaturaFaceFria;
exports.calcularTemperaturasInterfaces = calcularTemperaturasInterfaces;
exports.calcularEconomiaECO2 = calcularEconomiaECO2;
exports.calcularTemperaturaOrvalho = calcularTemperaturaOrvalho;
exports.calcularEspessuraMinimaCondensacao = calcularEspessuraMinimaCondensacao;
exports.calcularPerdaSemIsolante = calcularPerdaSemIsolante;
 ...
 */
function calcularK(kFuncStr, tMedia) {
    try {
 ...
 */
function calcularHConv(tf, to, geometria, outerDiameterM, windSpeedMs = 0) {
    const tfK = tf + 273.15;
    const toK = to + 273.15;
 ...
 */
function encontrarTemperaturaFaceFria(tQuente, tAmbiente, lTotalM, kFuncStr, geometria, emissividade, pipeDiameterM, windSpeedMs = 0) {
 ...
 */
function calcularTemperaturasInterfaces(tQuente, tFria, espessurasMm, kFuncStr, geometria, qComIsolanteWM2, diametroTuboMm) {
    if (espessurasMm.length <= 1)
        return [];
    const kMedio = calcularK(kFuncStr, (tQuente + tFria) / 2);
    if (kMedio === null || kMedio <= 0)
        return [];
 ...
 */
function calcularEconomiaECO2(perdaComIsolanteKwM2, perdaSemIsolanteKwM2, combustivel, custoCombustivel, areaM2, horasOperacaoDia, diasOperacaoSemana) {
    const { pc, ef, fatorEmissao } = exports.COMBUSTIVEIS[combustivel];
    ...
    const reducaoPercentual = perdaSemIsolanteKwM2 > 0 ? (economiaKwM2 / perdaSemIsolanteKwM2) * 100 : 0;
...
function calcularTemperaturaOrvalho(tAmbiente, umidadeRelativa) {
    const aMag = 17.27;
    const bMag = 237.7;
 ...
 * condensação.
 */
function calcularEspessuraMinimaCondensacao(tInterna, tAmbiente, umidadeRelativa, kFuncStr, geometria, pipeDiameterM, windSpeedMs = 0) {
    const temperaturaOrvalho = calcularTemperaturaOrvalho(tAmbiente, umidadeRelativa);
    for (let mm = 1; mm <= 500; mm++) {
        const lTesteM = mm / 1000;
        const { temperaturaFaceFria, convergiu } = encontrarTemperaturaFaceFria(tInterna, tAmbiente, lTesteM, kFuncStr, geometria, 0.9, pipeDiameterM, windSpeedMs);
...
function calcularPerdaSemIsolante(tQuente, tAmbiente, geometria, emissividade, pipeDiameterM, windSpeedMs = 0) {
