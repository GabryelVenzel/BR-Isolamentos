...
import type { FornecedorAnexo } from "@/lib/types/domain";

const BUCKET = "fornecedores-anexos";
...
/** Documentações do fornecedor (certidões, catálogos, contratos...) — só
 * disponível editando um fornecedor já existente, mesmo padrão de
 * ParceiroAnexos.tsx. */
export default function FornecedorAnexos({ fornecedorId }: Props) {
  const [anexos, setAnexos] = useState<FornecedorAnexo[]>([]);
  ...
  const carregar = useCallback(async () => {
    setCarregando(true);
    try {
      const response = await fetch(`/api/operacional/fornecedores/${fornecedorId}/anexos`);
  ...
  async function enviarArquivo(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (anexos.length >= LIMITE_ANEXOS) {
      toast.erro(`Limite de ${LIMITE_ANEXOS} anexos por fornecedor atingido.`);
      event.target.value = "";
      return;
    }
    ...
    try {
      ...
      const caminho = `${fornecedorId}/${Date.now()}-${arquivo.name}`;
  ...
  async function remover(anexo: FornecedorAnexo) {
    if (!confirm(`Remover "${anexo.nome_arquivo}"?`)) return;
    try {
      const supabase = createSupabaseBrowserClient();
      await supabase.storage.from(BUCKET).remove([anexo.storage_path]).catch(() => undefined);

      const response = await fetch(`/api/operacional/fornecedores/${fornecedorId}/anexos/${anexo.id}`, { method: "DELETE" });
      const payload = await response.json();
      if (!response.ok || !payload.success) {
  ...
  return (
    <div className="rounded-card border border-gray-200 p-4">
      <h3 className="mb-3 font-montserrat text-xs font-bold uppercase text-brand">
        📎 Anexos do Fornecedor ({anexos.length}/{LIMITE_ANEXOS})
      </h3>
