 ...
 * detalhe — os detalhes de cada área vivem nas abas Operação/Comercial/
 * Financeira ao lado, um clique adiante). Este era o conteúdo INTEIRO de
 * app/resumo/page.tsx antes da introdução das 4 abas — foi só extraído pra
 * um componente próprio, comportamento e chamadas de API inalterados.
 *
 * Não foi adicionada uma faixa extra de "insights rápidos" (leads dormindo,
 * serviços atrasados etc. como cards de texto) sugerida no pedido original:
 * essa mesma informação já aparece em 3 lugares — no AlertsBanner logo
 * acima (quando é crítica), nos gráficos desta própria aba, e em detalhe
 * completo na aba correspondente (Comercial mostra "Leads Dormindo",
 * Operação mostra "Serviços Vencidos"). Uma quarta cópia da mesma contagem
 * seria repetição, não uma visão nova. */
